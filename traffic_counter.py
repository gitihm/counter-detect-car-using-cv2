...
firebase = firebase.FirebaseApplication(os.getenv("FIREBASE"))
...
if __name__=='__main__':
    ...
    firebase.put('/user','in',0)
    firebase.put('/user','out',0)
    while True:
        ...
        if ret:  # if there is a frame continue with code
            ...
            cv2.rectangle(image, (0, 0), (250, 40), (255, 255, 255), -1)  # background rectangle for on-screen text

            # cv2.putText(image, "Cars in Area: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)

            cv2.putText(image, "Cars Crossed Up: " + str(carscrossedup), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0),
                        1)
            if carscrossedup != pev_in:
                update('in',carscrossedup,carscrosseddown)
                pev_in = carscrossedup
                # 

            cv2.putText(image, "Cars Crossed Down: " + str(carscrosseddown), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5,
                        (0, 170, 0), 1)
            if carscrossedup != pev_out:
                update('out',carscrossedup,carscrosseddown)
                pev_out = carscrosseddown
              
            # cv2.putText(image, "Total Cars Detected: " + str(len(carids)), (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5,
            #             (0, 170, 0), 1)
